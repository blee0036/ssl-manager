...
func (db *DB) Migrate() error {
	...
	// 撤销历史版本错误引入的 domains 规范化唯一索引：该索引及其配套的
	// CreateIfNotExists/reconcileApexDomainMonitorsFromCloudflare 逻辑（曾属于
	// cloudflare-domain-auto-sync 功能）基于一个错误假设——"Cloudflare 上存在
	// 该 Zone 就应该对其根域名做 TLS 证书监控"。这个假设是错的：TLS 监控的前提
	// 是该主机名有真实可解析的 A/AAAA/CNAME 记录，与"是否是一个 Zone/根域名"
	// 完全无关（域名注册到期监控 root_domains 才是天然按 Zone/根域名维度工作
	// 的地方）。同时该唯一索引本身还会在同一主机名有多条 DNS 记录（如多个 A
	// 记录，或同时有 A 和 AAAA）时阻止正常的 DNS 记录同步。
	// DROP INDEX IF EXISTS 对已经跑过旧版迁移、已建立该索引的库是幂等且安全的
	// 撤销；对从未建立过该索引的库是无操作。
	if _, err := db.Exec(`DROP INDEX IF EXISTS idx_domains_name_normalized`); err != nil {
		return fmt.Errorf("drop idx_domains_name_normalized: %w", err)
	}
