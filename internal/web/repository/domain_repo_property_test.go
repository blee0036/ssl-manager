...
// -----------------------------------------------------------------------------
// Task 2.3 (cloudflare-domain-auto-sync spec): CreateIfNotExists batch dedup
// idempotence property test
// -----------------------------------------------------------------------------
//
// setupDedupPropertyTestDB is used ONLY by the property test below. It is
// intentionally distinct from both the package-level setupPropertyTestDB
// (defined above in this file, no unique index) and the
// setupDomainTestDBWithUniqueIndex helper added by task 2.2 in
// domain_repo_test.go, to avoid any duplicate-symbol risk while that task is
// developed in parallel. CreateIfNotExists's
// `ON CONFLICT(LOWER(RTRIM(name, '.'))) DO NOTHING` clause requires the
// idx_domains_name_normalized UNIQUE index (mirroring internal/database/migrate.go)
// to exist as its exact conflict target.

// setupDedupPropertyTestDB creates an in-memory test DB with the domains table
// PLUS the idx_domains_name_normalized UNIQUE index on LOWER(RTRIM(name, '.')).
func setupDedupPropertyTestDB(t *testing.T) *DomainRepository {
	t.Helper()
	db := setupTestDB(t)
...
// dedupBaseNamePool / dedupCaseFns / dedupTrailingDotPool combine to produce
// dedupNameCombos distinct raw name strings that deliberately stress
// normalization/dedup: mixed-case labels and a variable number of trailing dots.
// Many distinct combos normalize (LOWER + RTRIM '.') to the same value (e.g.
// "ALPHA.COM", "alpha.com.", "AlPhA.cOm.." all -> "alpha.com").
var dedupBaseNamePool = []string{"alpha", "bravo", "charlie", "delta", "echo"}

var dedupCaseFns = []func(string) string{
	func(s string) string { return s },
	strings.ToUpper,
	dedupMixedCase,
}

var dedupTrailingDotPool = []string{"", ".", ".."}

// dedupNameCombos is the number of distinct raw names dedupNameFromSeed can
// produce; the seed generator ranges over [0, dedupNameCombos) so seeds map
// bijectively to combos while slice-level repetition (a seed appearing more
// than once in the generated slice) still produces literal duplicate names.
const dedupNameCombos = 5 * 3 * 3 // len(base)*len(caseFns)*len(dots) = 45

// dedupMixedCase alternates the case of each ASCII letter in s (index 0
// uppercased, index 1 lowercased, and so on), leaving non-letters untouched.
// It only needs byte-level ASCII manipulation, so it avoids introducing any new
// import into this file.
func dedupMixedCase(s string) string {
	b := []byte(s)
	for i := range b {
		c := b[i]
		switch {
...
// dedupNameFromSeed deterministically decodes a seed into a raw domain name of
// the form <case-variant of base.com><trailing dots>, e.g. "ALPHA.com..",
// "bravo.com.", "Charlie.com".
func dedupNameFromSeed(seed int) string {
	if seed < 0 {
		seed = -seed
	}
	base := dedupBaseNamePool[seed%len(dedupBaseNamePool)]
	seed /= len(dedupBaseNamePool)
	caseFn := dedupCaseFns[seed%len(dedupCaseFns)]
	seed /= len(dedupCaseFns)
	dots := dedupTrailingDotPool[seed%len(dedupTrailingDotPool)]
	return caseFn(base+".com") + dots
}

// dedupDomainRow is a minimal snapshot of a persisted domains row, used to
// compare the full record set before/after a repeated batch application.
type dedupDomainRow struct {
	name        string
	source      string
	monitorPort int
}

// snapshotDedupRows returns every current domains row keyed by its id.
func snapshotDedupRows(t *testing.T, repo *DomainRepository) map[string]dedupDomainRow {
	t.Helper()
	rows, err := repo.db.Query(`SELECT id, name, source, monitor_port FROM domains`)
	if err != nil {
		t.Fatalf("failed to query domains for snapshot: %v", err)
	}
	defer rows.Close()

	snapshot := make(map[string]dedupDomainRow)
	for rows.Next() {
		var id, name, source string
		var monitorPort int
		if err := rows.Scan(&id, &name, &source, &monitorPort); err != nil {
			t.Fatalf("failed to scan domain row for snapshot: %v", err)
		}
		snapshot[id] = dedupDomainRow{name: name, source: source, monitorPort: monitorPort}
	}
	if err := rows.Err(); err != nil {
		t.Fatalf("error iterating domain rows for snapshot: %v", err)
	}
	return snapshot
}

// sameDedupSnapshots reports whether two row-id-keyed snapshots contain
// exactly the same ids mapped to exactly the same field values.
func sameDedupSnapshots(a, b map[string]dedupDomainRow) bool {
	if len(a) != len(b) {
		return false
	}
	for id, rowA := range a {
...
// dedupAtMostOnePerNormalizedName asserts that no normalized name
// (LOWER+RTRIM '.') has more than one row in the domains table, logging any
// violation it finds before returning false.
func dedupAtMostOnePerNormalizedName(t *testing.T, repo *DomainRepository) bool {
	t.Helper()
	rows, err := repo.db.Query(`SELECT LOWER(RTRIM(name, '.')) AS normalized, COUNT(*) AS cnt
		FROM domains GROUP BY normalized HAVING COUNT(*) > 1`)
	if err != nil {
		t.Fatalf("failed to query for duplicate normalized names: %v", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var cnt int
		if err := rows.Scan(&normalized, &cnt); err != nil {
			t.Fatalf("failed to scan duplicate-check row: %v", err)
		}
		t.Logf("normalized name %q has %d rows (expected at most 1)", normalized, cnt)
		ok = false
	}
	if err := rows.Err(); err != nil {
		t.Fatalf("error iterating duplicate-check rows: %v", err)
	}
	return ok
}

// Feature: cloudflare-domain-auto-sync, Property 1: 批量 CreateIfNotExists 去重幂等
// **Validates: Requirements 2.2, 2.3**

func TestProperty_BatchCreateIfNotExistsDedupIdempotent(t *testing.T) {
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 100
	properties := gopter.NewProperties(parameters)

	nameSeedsGen := gen.SliceOf(gen.IntRange(0, dedupNameCombos-1))

	properties.Property("batch CreateIfNotExists dedups by normalized name and repeating the batch is a no-op", prop.ForAll(
		func(seeds []int) bool {
			repo := setupDedupPropertyTestDB(t)
			ctx := context.Background()

			names := make([]string, len(seeds))
			for i, s := range seeds {
				names[i] = dedupNameFromSeed(s)
			}

			// applyBatch sequentially calls CreateIfNotExists for every name in the
			// batch. A conflict (created=false) is the expected dedup outcome, not
			// an error; only a genuine DB failure returns false here.
			applyBatch := func() bool {
				for _, name := range names {
					...
					if _, err := repo.CreateIfNotExists(ctx, domain); err != nil {
						t.Logf("CreateIfNotExists failed for %q: %v", name, err)
						return false
					}
				}
				return true
			...
			if !dedupAtMostOnePerNormalizedName(t, repo) {
				return false
			}
			snapshotAfterOnce := snapshotDedupRows(t, repo)

			// Re-apply the exact same batch a second time.
			if !applyBatch() {
				return false
			}
			if !dedupAtMostOnePerNormalizedName(t, repo) {
			...
			snapshotAfterTwice := snapshotDedupRows(t, repo)

			// Idempotence: the final persisted record set after applying the batch
			// once equals the final persisted record set after applying it twice.
			if !sameDedupSnapshots(snapshotAfterOnce, snapshotAfterTwice) {
				t.Logf("repeating the batch changed persisted rows: after-once=%v after-twice=%v",
					snapshotAfterOnce, snapshotAfterTwice)
				return false
			}
